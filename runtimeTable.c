...
int main( int argc, char *argv[] )
{
    ...
    //int sizes1[] = { 500, 1000, 2000, 4000, 8000, 16000, 32000, 64000, 128000, 256000 }; //test code, change in final
 ...
 * Fill in the missing parts of the code (see TODOs below) //DONE
 */
functionRuntimes timeAlgorithm( char *szName, int iNumRepeats, int iNumTestCaseSizes, int arrTestSizes[], void (*f)(FILE *) )
{
    ...
    /* fill szName in fRT with the variable szName */ //DONE
    /* fill iNumRepeats in fRT with the variable iNumRepeats */ //DONE
    /* fill iNumTestCaseSizes in fRT with the variable iNumTestCaseSizes */ //DONE
    /* malloc space for arrTestSizes in fRT to hold iNumTestCaseSizes number of ints */ //DONE
    /* fill arrTestSizes in fRT with the variable arrTestSizes (hint: use a loop) */ //DONE


    //fills szName in fRT with variable szName
    fRT.szName =szName;
    //fills fRT.iNumRepeats with data from iNumRepeats arg
    fRT.iNumRepeats=iNumRepeats;
    //fills fRT.iNUmTestCaseSizes with data from iNumTestCaseSizes arg
    fRT.iNumTestCaseSizes=iNumTestCaseSizes;

    //mmmm yummy malloc for TestCaseSizes
    fRT.arrTestSizes= malloc(sizeof(int)*iNumTestCaseSizes);

    //fill the arrTestSizes
    int n;
    for(n=0; n < iNumTestCaseSizes;n++) {
        fRT.arrTestSizes[n]=arrTestSizes[n];
    }
    
    //TODO: malloc an array with iNumTestCaseSizes variables of type double* (on next line) //DONE
    fRT.arrRuntimes = (double**) malloc(iNumTestCaseSizes * sizeof(double*)); /* replace NULL with your code */

    for( i=0; i<iNumTestCaseSizes; i++ )
    {
        //TODO: malloc an array with iNumRepeats variables of type double (on next line) //DONE
        fRT.arrRuntimes[i] =  malloc(iNumRepeats * sizeof(double)); /* replace NULL with your code and uncomment the line */
        for( j=0; j<iNumRepeats; j++ )
        {
            ...
            //TODO: uncomment the next line line after you've malloc-ed memory for fRT.arrRuntimes //DONE
            fRT.arrRuntimes[i][j] = (double)(end - start) / CLOCKS_PER_SEC;
        }
    }

    //TODO: on the next line, malloc space for fRT.arrAvg (you'll need to store iNumTestCaseSizes variables of type double) //DONE
    fRT.arrAvg = (double*) malloc(sizeof(double)*iNumTestCaseSizes); /* replace NULL with your code */
    //TODO: Calculate the average runtimes (i.e. call computeAvg here) //DONE
    computeAvg(fRT);
 ...
 * Calculate and insert the average runtime for each set of test data into fRT //DONE
 */
void computeAvg( functionRuntimes fRT )
{
    int i, j;
    ...
    for(i=0;i<fRT.iNumTestCaseSizes;i++) {
        ...
        for(j=0;j<fRT.iNumRepeats;j++) {
            sum= (double) sum+ fRT.arrRuntimes[i][j];
        }
        fRT.arrAvg[i]=sum/fRT.iNumRepeats;
    }
}

/* TODO: TO BE COMPLETED BY YOU //DONE
...
void printRuntimeTable( functionRuntimes fRT )
{
    int i, j;
    double increase;
    //prints out the title and starts second line
    printf("%s \nTest Size    ",fRT.szName);

    //continues to add tests until iNumRepeats is reached
    for(i=0;i<fRT.iNumRepeats;i++) {
            printf("Test #%-5i",i+1);
    } //
    printf("Average   Increase\n");

    /*This nexted forloop handles the columns of the loop*/
    for(i=0;i<fRT.iNumTestCaseSizes;i++) {
        
        printf("%-*d", 13,fRT.arrTestSizes[i]); //print out the test size

        
        for(j=0;j<fRT.iNumRepeats;j++) {  //print out all test runtimes
            printf("%-11.3f",  fRT.arrRuntimes[i][j]);
        }
        printf("%-11.3f", fRT.arrAvg[i]);//print out the average

        //get increase if/else
        if(i==0) //print n/a if first row
            printf("%-11s", "N/A");

        else {//calculate the increase then print it out
            increase=fRT.arrAvg[i]/fRT.arrAvg[i-1];
            printf("%-11.3f", increase);
        }
        printf("\n"); //print new line for next test size
    }
    printf("\n");//print blank line for next loop :)
}

/* TODO: TO BE COMPLETED BY YOU
 * Free all of the dynamically allocated memory in fRT //DONE
 */
void freeFunctionRuntimes( functionRuntimes fRT )
{
    int i;
    ...
    free(fRT.arrTestSizes);     //array containing the test case sizes
}
